...
const ProductDetailsCarousel = ({ colorFromURL, product }) => {
  ...
  if(product.images) {
    ...
    colorImages = imageObjArray[imageObjArray.findIndex(index => index.color === colorFromURL)].colorImages.map(i => i.source);
    combinedImagesForCarousel = colorImages.concat(product.defaultImages);
